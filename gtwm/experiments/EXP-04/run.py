"""薄いラッパー。ロジックは `gtwm.eval.experiments.exp04` / `gtwm.eval.runner` に置く。"""
...
from gtwm.eval.experiments.exp04 import measure
from gtwm.eval.runner import generate_report, run
from gtwm.utils.config import load_config, to_container

EXP_ID = "EXP-04"
CONFIG_PATH = "experiments/EXP-04/config.yaml"
PURPOSE = (
    "整合性ギャップ ε がプロセス逸脱（手順変更）の検知指標として機能するかを検証する"
    "（H3、poc_plan.md 3.1）"
)
_CRITERIA_KEYS = ("drift_detection_auroc", "epsilon_daily_cv")


def main(run_timestamp: str) -> None:
    config = load_config(CONFIG_PATH)
    all_criteria = to_container(load_config("experiments/criteria.yaml"))
    criteria = {k: v for k, v in all_criteria["H3"]["metrics"].items() if k in _CRITERIA_KEYS}
    ...
    generate_report(
        ...
        criteria,
        full_run_note=(
            "本実行には2週間分の平常運転ログ＋3日間の手順変更後ログ（config.yaml の "
