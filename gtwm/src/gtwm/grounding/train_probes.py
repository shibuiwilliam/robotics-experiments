...
from gtwm.wm.dataset import list_episodes, read_frames_with_retry
...
def _preload_frames(samples: list[AnchorSample]) -> FrameCache:
    ...
    for eid, (rep_sample, frame_idxs) in by_episode.items():
        ...
        for cam in ep.cameras:
            ...
            frames = read_frames_with_retry(path, sorted_idxs)
            for idx, frame in zip(sorted_idxs, frames, strict=True):
                per_cam[idx].append(frame)
