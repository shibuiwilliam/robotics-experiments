...
| EXP-04 | H3（ε とドリフト検知） | **本実行済み（seed×3, F^h修正後の初回, 約28時間）** | `runs/EXP-04/20260914-143908`：drift_detection_auroc=**0.472（偶然水準、seed別0.32/0.68/0.42と符号も不安定）**, epsilon_daily_cv=0.111（**達成**）。**根本原因を特定**：`sim/drift.py` の `inspection_position_change` は `inspecting` イベントが実データで0件のため**完全なno-op**、残る `picking_order_change` も343行中18行（5%）しか変更しない＝**ドリフトが実質注入されていない退行実験**（EXP-06と同型）。H3は反証されたのではなく未検証。詳細: `docs/results/EXP-04.md` | **不合格** |
...
| EXP-11 | N1（非機能） | **本実行済み（seed×3, 60トライアル, 約118時間）** | `runs/EXP-11/20260914-033140`：e2e_latency_p50_s=**385.5**（目標1.0秒／バッチ実装のためエピソード長に比例する定義上の帰結、ストリーミング化が前提条件）, availability=**0.717**（60中17失敗／原因が`except Exception: continue`で握り潰され事後特定不能——本レポート作成時に`failure_types_csv`とログ出力を追加して対処）, monthly_cost_per_zone=$180（report_only）。詳細: `docs/results/EXP-11.md` | **不合格** |
| EXP-08 | H7（概念発見） | **本実行済み（seed×3, 実LLM=gpt-5.6-luna）** | `runs/EXP-08/20260913-170237`：injected_concept_top5_hit=3/3種（機械的には達成）だが**要注意**：n_candidates=1のみで、その1候補が時間的近接性の粗い判定により3種全てに「命中」——smokeのような複数独立候補による発見ではない（詳細: docs/results/EXP-08.md） | **合格（機械的、但し実質性に疑義あり）** |
