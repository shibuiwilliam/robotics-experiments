...
| **Engine wired per-arm** | ✅ done | `drivers.py::_select_planner` honors the SCENARIOS.md §5 arm contract: A0/A1 plan via `ScriptedPlanner`, **A2–A4 plan via the LLM (Claude) engine** offline through `FakeGeminiClient` (passthrough VCR). Shared `agents/grounding.py::ground_relocate` grounds both planners' skeletons identically, so the ablation isolates the *reasoning backend*, not geometry. Driver + console `inspect` surface `planner`/`provider`/`llm_calls`; `api_calls=0` (E0: 15/15 oracle, A2–A4 `llm:claude`, 0 API). 3 added tests. |
