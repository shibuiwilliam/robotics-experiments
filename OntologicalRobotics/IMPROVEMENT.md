...
- **必須要件（全件実装）**: 上表の **❌ 未着手 4 件（S3, S7, S4, S5）を必ず全て実装する**。
...
| C3 **全シナリオ S1–S7 完全実装** | ⏳ 進行中（**3/7**: S1,S2,S6✅・M-Scenario-A✅） | **残 4 件 S3→S7→S4→S5 を必ず実装**。完了定義=`make scenario-validate` 全件緑＋`orx scenario list` 全件 implemented |
