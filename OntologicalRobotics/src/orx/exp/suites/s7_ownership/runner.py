...
def _eval_seeds(world: S7World, seeds: list[int], sep: float) -> dict[str, S7ConditionAgg]:
...
def run(config: ScenarioExperimentConfig, exp_dir: Path, notify: object) -> dict:
    ...
    per_condition = _eval_seeds(world, config.seeds, sep)
    for c in CONDITIONS:
        a = per_condition[c]
        notify_fn(
    ...
    robustness = {
        ...
        "misdelivery_rate": misdeliv_curve, "success_rate": success_curve,
    }
...
def demo(runs_root: Path, notify: object) -> tuple[dict, str]:
    config = ScenarioExperimentConfig(
        name="s7-demo", scenario="s7", world_config=WORLD, conditions=CONDITIONS,
        seeds=[701, 702, 703, 704], duration_s=0.0,
        knob="lookalike_sep", knob_values=[1.5, 1.0, 0.6, 0.3],
...
def render_report(result: dict) -> str:
    ...
    lines += ["", f"### 条件別サマリ（sep={result['primary_sep']}）", "",
