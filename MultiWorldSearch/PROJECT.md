...
- **新規 LLM／埋め込みモデルの学習**：基盤モデルはクラウド（Gemini）に依存する。例外として、ローカル埋め込みの蒸留・整列（H7）は研究対象に含む。
...
| 埋め込み（権威的・オフライン） | クラウド | Gemini Embedding 2（ネイティブマルチモーダル、3072次元、MRL 切り詰め、Batch API で非同期・低コスト） |
| 埋め込み（ホットパス・テキスト） | ローカル | EmbeddingGemma（768→128 MRL、オンデバイス、低レイテンシ・低RAM） |
...
- **権威的埋め込み（Gemini Embedding 2）はオフライン索引化**に用い、Batch API で非同期・低コスト化する。
- **ホットパス（テキストクエリ）はローカル（EmbeddingGemma）**で処理し、content-hash キーの埋め込みキャッシュと非同期インデクサでクラウド往復を外す。
- マルチモーダル（画像→共有空間）は Gemini 依存のため、視覚アトムにはクラウド遅延が残る。これ自体が研究対象（仮説 H7）である。
...
2. **教師／生徒ハイブリッド埋め込み**：Gemini Embedding 2 を権威的教師、EmbeddingGemma をホットパス生徒として整列させ、ローカル低遅延とクラウド高品質を両立する仕組みそのものを研究貢献とする。
...
| H7 | 教師(Gemini)/生徒(local) 二層埋め込みがレイテンシ↔品質を改善する | 全般（環境固有） |
