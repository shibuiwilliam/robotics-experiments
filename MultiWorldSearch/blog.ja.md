...
つまり、**metrics に計上されていないクラウド呼び出しは存在しません**。LLM のトークン数も推定値ではなく、API が返す usage_metadata の**実測値**です（18 呼すべて。応答本文も `llm_calls.jsonl` として各実走の成果物に記録されます。ちなみに実測導入時、従来の「文字数 ÷ 4」推定が入力トークンを約 2.6 倍過大評価していたことが判明し、コストを下方修正しました）。
...
応答記録も、早速役に立ちました。live の LLM は同じ seed でも揺らぎます。実際、ある実走では保全計画が 8 ステップ、次の実走では 7 ステップになりました。従来ならこの変動は「そういうこともある」で終わりですが、`llm_calls.jsonl` に**計画の本文がそのまま残っている**ため、「この走の 8 ステップはこの応答だ」と成果物から逐語的に確認できます。実際、本記事の実走で実 LLM 呼が前走の 17 から 18 に増えたときも、記録された計画を読むだけで「今回は 8 ステップ計画だったため」と**数秒で**説明がつきました。

さらに、各実走の `manifest.json` には git SHA とその**ダーティ/未追跡状態**、モデル ID、埋め込みバッチサイズ、主要依存のバージョンが記録されます。つまり**「どのコードで・どのモデルで・どの設定で走ったか」が manifest 単体から特定できます**。この dirty フラグも飾りではありません。ある実走では `git_dirty: true` が記録され、調べるとレポートファイル 1 つが作業ツリーから削除されていたことが原因でした（コードは SHA と完全一致）。manifest はコードの変更だけでなく、**ドキュメント 1 ファイルの削除まで正直に拾います**。
...
**ゲート方法**: 計画の各ステップの「完了」は、**そのステップが必要とする根拠アトムが検索に出てきたか**で判定します。たとえば「SOP 取得」ステップは、SOP アトムが検索結果に含まれて初めて完了になります。今回の実走では LLM が 8 ステップの計画を生成し（計画本文は `llm_calls.jsonl` に記録）、8/8 がエビデンスゲートを通過しました。**壊し方**: SOP アトムを記憶から外すと、当該ステップは未完了になり `all_steps_complete` が False に落ちます（テスト済み）。エージェントの“やったふり”は通りません。
...
| gemini-3.5-flash 呼び出し（ADK） | **実 18**（全件実測トークン・全応答を llm_calls.jsonl に記録）＋コストモデル計上 11 |
| 推定クラウドコスト | **約 $0.0046**（実測トークン基準） |
| エッジ↔クラウド帯域（実/仮想） | 912 KB / 166 KB |
| 監査ログ件数 | 100 |

この数値は前述のとおり**3点照合監査でゼロ差分**（embedding 64/64・実 LLM 18/18・記録 18/18）を確認したものです。

注目したいのは、**コストもレイテンシも LLM 推論が支配的**だという点です。レイテンシ分解の実測では、ローカル ANN 検索が **0.04 ミリ秒**、Gemini 埋め込みが p50 **約 370〜410 ミリ秒**、Gemini 推論が p50 **2.8〜4.2 秒**（p95 は最大 6.8 秒）。S1 の `act` フェーズは独立ステップの並列実行（既定 4 並列）で約 12〜17 秒です（計画の呼数に依存。逐次実行時は約 36 秒でした）。一方、取り込み側は複数テキストを 1 リクエストにまとめるバッチ埋め込みを全シナリオに適用し、知覚フェーズは **3.7〜6.7 秒 → 0.4〜0.9 秒**（最大 -92%）に短縮されています。共通プロトコルのボトルネックは検索そのものではなく、クラウド LLM の往復にあります。
...
- **manifest は「dirty かどうか」までしか記録しません。** どのパスが dirty かの一覧は未記録で、原因特定には git が必要でした（改善項目 M20 として [IMPROVEMENT.md](IMPROVEMENT.md) に登録済み）。
...
*本記事の検証は MuJoCo シミュレーション環境で実施。LLM（gemini-3.5-flash）と embedding（gemini-embedding-2, 768d, 非対称タスク指示・v2 空間）は実際の Gemini API、それ以外はシミュレーション／合成データ。シナリオ数値は seed=0 の live 実走（git 77b7ee3・manifest にモデル/依存/ツリー状態を記録）から取得し、クラウド呼び出し数は3点照合監査でゼロ差分（embedding 64/64・実 LLM 18/18・記録 18/18）。アブレーションは S1 の代表クエリに対するもの。各シナリオは入力を変えると仮説が崩れる反証可能性テストを伴う。詳細は [REPORT.md](REPORT.md)、未解決の計測課題は [IMPROVEMENT.md](IMPROVEMENT.md) を参照。*
