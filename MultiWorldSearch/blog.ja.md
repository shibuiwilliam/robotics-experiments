...
LLM はこれらの検索結果をもとに 8 ステップの保全計画を生成しました。ケーシングの取り外し手順には「SOP-PUMP-07 に従い、トルク仕様 **45 Nm** に注意」と記載されています。
...
そして、**それぞれに「入力を変えると仮説が崩れる」反証可能性テストを付けました。** 物理観測を古くすれば在庫上書きは起きない。SOP 上限を緩めれば危険操作が「実行可」に反転する。ノイズを増やせばパターン発見は失敗する。把持力の上限を下げれば、スキルを思い出しても作業は失敗する——これらが実際にテストとして緑になることを確認しています（mock モードで全 255 テストがパス）。
...
「クラウド呼び出しは 123 回でした」という数値は、どうやって信じればよいでしょうか。MWS では、実際のクラウド呼び出しが必ずログに 1 行のマーカーを残すようにし、**実走ログのマーカー行数と metrics の計上値を突き合わせる監査ツール**（`mws eval reconcile`）を常設しました。差分が 1 件でもあれば非ゼロ終了します。本記事の実走では:

```
actual : embedding_requests=123  llm_calls_real=18   （ログの実呼び出し行数）
counted: embedding_requests=123  llm_calls_real=18   （metrics の計上値合計）
delta  : 0 / 0  →  reconciled: true
```

つまり、**metrics に計上されていないクラウド呼び出しは存在しません**。LLM のトークン数も推定値ではなく、API が返す usage_metadata の**実測値**です（18 呼すべて。ちなみに実測導入時、従来の「文字数 ÷ 4」推定が入力トークンを約 2.6 倍過大評価していたことが判明し、コストを下方修正しました）。
...
なお Recall@5 は 0.50 のまま改善しておらず、上位 5 件の精度向上は今後の課題です。
...
**ゲート方法**: 計画の各ステップの「完了」は、**そのステップが必要とする根拠アトムが検索に出てきたか**で判定します。たとえば「SOP 取得」ステップは、SOP アトムが検索結果に含まれて初めて完了になります。今回の実走では LLM が 8 ステップの計画を生成し、8/8 がエビデンスゲートを通過しました。**壊し方**: SOP アトムを記憶から外すと、当該ステップは未完了になり `all_steps_complete` が False に落ちます（テスト済み）。エージェントの“やったふり”は通りません。
...
| 項目 | 値 |
|------|----|
| Gemini Embedding（768d・非対称タスク指示） | **123 リクエスト / 163 テキスト**（マルチ Content バッチ化でリクエスト -25%） |
| gemini-3.5-flash 呼び出し（ADK） | **実 18**（全件実測トークン）＋コストモデル計上 11 |
| 推定クラウドコスト | **約 $0.0046**（実測トークン基準） |
| エッジ↔クラウド帯域（実/仮想） | 908 KB / 166 KB |
| 監査ログ件数 | 100 |

この数値は前述のとおり**実走ログとの突き合わせ監査でゼロ差分**を確認したものです。

注目したいのは、**コストもレイテンシも LLM 推論が支配的**だという点です。レイテンシ分解の実測では、ローカル ANN 検索が **0.04 ミリ秒**、Gemini 埋め込みが p50 **約 370〜410 ミリ秒**、Gemini 推論が p50 **2.8〜4.2 秒**（p95 は最大 6.8 秒）。S1 の `act` フェーズは逐次の ADK 呼び出しで約 36 秒かかりました。一方、取り込み側は複数テキストを 1 リクエストにまとめるバッチ埋め込みの導入で、S1 の知覚フェーズが **6.7 秒 → 0.83 秒**に短縮されています。共通プロトコルのボトルネックは検索そのものではなく、クラウド LLM の往復にあります。
...
**二つ目の解が、教師側を仕様どおりに使い切ることです。** 前述の非対称タスク指示（A/B で Recall@5 +0.056）に加え、取り込みは複数テキストを 1 リクエストにまとめるバッチ埋め込み（リクエスト -25%・知覚フェーズ -87%）、オフラインの再索引は **Batch API**（標準価格の 50%・非同期ジョブ・中断再開可能）に逃がしています。Batch API の実ジョブも投入から取り込みまで完走させ、得られたベクトルが同期経路と一致することを確認済みです。
...
- **Recall@5 が伸びません。** Recall@10 は高いのに上位 5 件では半分程度。RRF の重みチューニングが課題です。
- **アブレーションで「＋構造化」が逆効果**でした。インデックスを足せば必ず良くなるわけではありません。
- **LLM 推論が支配的なボトルネック**です（p50 2.8〜4.2 秒、逐次実行）。並列化は未対応です。
- **知覚税（H3）は小さいが存在します。** 真値オラクル検索器との差分は Recall@10 で 0.10。知覚・記述・埋め込みの劣化で上位 10 件から関連アトムが 1 件こぼれています。
- **live の LLM は揺らぎます。** 同じ seed でも計画が 7 ステップの走と 8 ステップの走がありました（エビデンスゲートはどちらも全数通過）。応答本文を記録していないため事後検証ができず、これは計測ギャップとして [IMPROVEMENT.md](IMPROVEMENT.md) に登録しています。
- **H7 の生徒モデルは EmbeddingGemma ではありません。** HF のゲート制限のため、MiniLM-384 での検証です。
- **live の信頼区間は旧スキーム時点の値です。** 多シード CI は mock では常設、live は接頭辞導入前の計測（知覚税 0.100±0 等）で、新スキームでの再計測はこれからです。
...
**クラウド境界**: LLM 推論と embedding 生成のみクラウド。検索エンジン本体はすべてローカルです。MacBook 1 台で動作し、**255 件のテスト**で品質を担保しています。`MWS_CLOUD_MODE=mock` なら API キー不要・オフラインでも全シナリオを完走できます（決定的）。
...
- **クラウド埋め込みのレイテンシ問題には設計で答えられます。** ローカル生徒で 27.4 倍の高速化（品質維持）、API を仕様どおり使う非対称埋め込みで品質 +0.056、バッチ化で取込 -87%。ボトルネックとして残るのは LLM 推論の往復です。
...
*本記事の検証は MuJoCo シミュレーション環境で実施。LLM（gemini-3.5-flash）と embedding（gemini-embedding-2, 768d, 非対称タスク指示・v2 空間）は実際の Gemini API、それ以外はシミュレーション／合成データ。シナリオ数値は seed=0 の live 実走（git 48d2026）から取得し、クラウド呼び出し数は実走ログとの突き合わせ監査でゼロ差分（embedding 123/123・実 LLM 18/18）。アブレーションは S1 の代表クエリに対するもの。各シナリオは入力を変えると仮説が崩れる反証可能性テストを伴う。詳細は [REPORT.md](REPORT.md)、未解決の計測課題は [IMPROVEMENT.md](IMPROVEMENT.md) を参照。*
