...
## 🆕 P11 — 2026-06-12 検証本走（P10 完了後の `make scenario-all`）で判明した残ギャップ

> live 実走は 7/7 PASS・$0.00448・ゼロ差分（embedding 64/64・実LLM 17/17・全応答記録）。manifest は `git_sha=9e895f0b501e, dirty=false` で完全再現可能。S1 計画長の揺らぎ（今走7・前走8）を **llm_calls.jsonl の記録本文から逐語検証できた**（M16 の実証）。残ギャップは1点＋既知任意1点。

### M19 — 応答記録そのものが監査対象外（**Low-Medium・監査の完全性**）

- **現状**: `mws eval reconcile` は「ログのマーカー行数 ↔ metrics 計上値」の2点照合。M16 の `llm_calls.jsonl` は照合対象外のため、**レコーダの故障（配線漏れ・例外握りつぶしへの将来の改変）が起きてもゼロ差分のまま検出されない**。記録は「あるはず」で信頼されている。
...
### （既知・任意）
- `MWS_LLM_REPLAY`（記録応答の決定的リプレイ）は未実装 — 記録のみで M16 受け入れは満たす。応答固定の A/B 比較を行う段になったら実装。
