...
- **事前登録実験** — E1 接頭辞 A/B（+0.056）・H7（27.4×）・融合重みスイープ（現行確定・R@5 構造上限 93% の発見）。
...
H7 支持（27.4×・品質比1.06、MiniLM 代替）/ H8 支持 / H9 支持（CI 分離）/ E1 支持（+0.056）。
...
A2A 未実装。EmbeddingGemma は HF gated（唯一の外部ブロック）。Batch API token_count None。
